...
{
	class NanakoBitLaser : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			if (Projectile.ai[0] >= 4)
			{
				if (fluctuationTimer < 380)
				{
					Projectile.scale = 4.5f;
				}
				else
				{
					Projectile.scale = Matht.Lerp(4.5f, 2, (fluctuationTimer - 380.0f) / 20.0f);
				}
			...
			{
				// explodey sprite
				AccelerationHelper.DrawSprite("Acceleration/Projectiles/beam_impact", Projectile.Center, 0, 64, new Color(255, 0, 255, 255), Projectile.rotation, new Vector2(Projectile.scale, Projectile.scale), Main.spriteBatch);
				AccelerationHelper.DrawSprite("Acceleration/Projectiles/beam_impact", Projectile.Center, 0, 64, new Color(255, 0, 128, 255), Projectile.rotation, new Vector2(Projectile.scale, Projectile.scale) * 0.75f, Main.spriteBatch);
			}
			return false;
		}
